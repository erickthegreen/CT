...
class AtendimentoApp:
    def __init__(self, root):
        ...
        self.fonte_atual = font.Font(family="Arial", size=self.tamanho_fonte_base, weight="bold")
        self.fonte_bold_atual = font.Font(family="Arial", size=self.tamanho_fonte_base, weight="bold")
        self.fonte_entry_bold = font.Font(family="Segoe UI", size=11, weight="bold")
...
# --- INÍCIO DO NOVO SISTEMA DE ANIMAÇÃO UNIFICADO ---
    ...
    def atualizar_valores_religacao(self, event=None):
        ...
        try:
            ...
            # Corrige a nomenclatura "Trifásico" / "Trifásica" para acessar corretamente o dicionário
            tipo_instalacao_norm = {"Trifásico": "Trifásica"}.get(tipo_instalacao, tipo_instalacao)
            ...
            if estado_selecionado and tipo_instalacao and tipo_religacao:
                ...
                taxa = self.taxas_religacao_por_estado[estado_selecionado][tipo_religacao][tipo_instalacao_norm]
            ...
            # Se a combinação não existir ou algum widget ainda não estiver criado,
            # preenche os campos com "Indisponível"
            for campo in ("VALOR DE SERVIÇO TAXADO", "PRAZO DO SERVIÇO"):
                if campo in self.entries:
                    entry = self.entries[campo]
                    entry.config(state="normal")
                    entry.delete(0, tk.END)
                    entry.insert(0, "Indisponível")
                    entry.config(state="readonly")
            logger.warning(f"Combinação de estado/tipo não suportada ou widget ausente: {e}")
        except Exception as e:
    ...
    def processar_servico_6(self, servico_id):
        ...
        # Perguntas Sim/Não
        perguntas_radio = ["CASA MURADA", "TEM MEDIDOR"]  # <- tire "TEM HORÁRIO ESPECÍFICO..."
        # E depois, trate o horário como Entry comum:
        horario = self._get_entry_value("TEM HORÁRIO ESPECÍFICO PARA A FRAUDE")
        texto += f"TEM HORÁRIO ESPECÍFICO PARA A FRAUDE: {horario}\n"
    ...
    def adicionar_campo(self, nome_campo, parent=None):
        ...
        entry = ttk.Entry(frame, width=50, font=self.fonte_entry_bold) 
        entry.pack(side="left", fill="x", expand=True)
    ...
    def criar_form_reclamacao_unificado(self):
        ...
        # --- CORREÇÃO: Pega as cores do tema atual para aplicar nos widgets ---
        style = ttk.Style()
        bg_text = style.lookup("TEntry", "fieldbackground")
        fg_text = style.lookup("TEntry", "foreground")
        insert_color = fg_text # Cor do cursor de digitação
        ...
        self.desc_reclamacao_text.configure(bg=bg_text, fg=fg_text, insertbackground=insert_color)
        ...
        # Aplica as cores do tema escuro
        self.analise_atendente_text.configure(bg=bg_text, fg=fg_text, insertbackground=insert_color)
